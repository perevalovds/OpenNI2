...
XnStatus PlayerCodecFactory::Create(XnCodecID nCodecID, PlayerSource* pSource, XnCodec** ppCodec)
{
	...
	switch (nCodecID)
	{
		...
		case XN_CODEC_16Z_EMB_TABLES:
		{
			...
			rc0 = pSource->GetProperty(ONI_STREAM_PROPERTY_MAX_VALUE, &nMaxDepth, &dataSize);
			if (rc0 != ONI_STATUS_OK)
			{
		...
		case XN_CODEC_JPEG:
		{
			...
			rc0 = pSource->GetProperty(ONI_STREAM_PROPERTY_VIDEO_MODE, &videoMode, &dataSize);
			if (rc0 != ONI_STATUS_OK)
			{
				return XN_STATUS_ERROR;
			}
